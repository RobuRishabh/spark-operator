...
func buildKustomize(t *testing.T) []unstructured.Unstructured {
	t.Helper()
	return buildKustomizePath(t, filepath.Join("..", "..", "config", "default"))
}

// buildKustomizePath runs "kubectl kustomize" on the given directory and returns
// the parsed Kubernetes resources. The test is skipped when kubectl is absent.
func buildKustomizePath(t *testing.T, kustomizeDir string) []unstructured.Unstructured {
	t.Helper()
