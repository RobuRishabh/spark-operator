...
| **Invalid fsGroup** | ValidatingAdmissionPolicy rejects SparkApplications with fsGroup |
...
- **go** - Required by `make kind-setup` to install the Kind binary via `go install`
...
| `make test-invalid-fsgroup` | Run ValidatingAdmissionPolicy fsGroup test |
| `make test-all` | Run all tests (operator-install + spark-pi + docling + invalid-fsgroup) |

---

## Configuration Options

All test targets (`operator-install`, `test-spark-pi`, `test-docling-spark`, `test-invalid-fsgroup`) support the `CLEANUP` environment variable:
...
2. SparkApplication with fsGroup in podSecurityContext is rejected
3. Error message references the correct policy
...
- name: Run Docling Spark test
  run: CLEANUP=false make test-docling-spark

- name: Run invalid fsGroup test
...
| `test-invalid-fsgroup.sh` | Tests ValidatingAdmissionPolicy fsGroup rejection |
| `spark-pi-app.yaml` | SparkApplication manifest for Spark Pi |
| `invalid-fsgroup.yaml` | SparkApplication manifest with invalid fsGroup (for testing) |
